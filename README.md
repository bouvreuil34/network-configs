...
В Shadowrocket 2.2.90: **Настройки → Обновить → Настройка → Автообновление в фоновом режиме**, интервал **1 день**. После перезагрузки устройства или принудительного закрытия снова откройте приложение: по предупреждению Shadowrocket фоновые задачи сами не возобновляются.

Чтобы применить изменение `main` сразу: **Настройка → нужный профиль → Обновить**, дождитесь загрузки, затем **Использовать конфигурацию**. Проверьте галочку активного профиля и изменённую строку через **Редактировать обычный текст**; правила можно проверить через **ⓘ → Правило тестирования**. `update-url` задаёт адрес загрузки, но сам по себе не подтверждает, что новая версия загружена и используется.

`no-russia` подключён через `RULE-SET` к [no-russia.list](shadowrocket/no-russia.list): одна запись `DOMAIN-SUFFIX` охватывает домен и его поддомены. В 2.2.90 нативный `DOMAIN-SET` понимает `.domain`, но upstream публикует только обычные имена, `*domain` и `+.domain`; готовые wildcard-варианты практический тест не прошли. Поэтому [скрипт](scripts/update-no-russia.py) без сторонних библиотек преобразует только [upstream hosts.txt](https://raw.githubusercontent.com/dartraiden/no-russia-hosts/master/hosts.txt). [Update no-russia](.github/workflows/update-no-russia.yml) ежедневно получает список и коммитит только его изменения; доступен ручной **Actions → Update no-russia → Run workflow**. Ошибка загрузки, пустой или некорректный список сохраняют предыдущий файл. Домены вручную не копируются; свежие правила попадают в приложение при обновлении конфигурации.
